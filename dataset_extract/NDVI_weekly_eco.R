...
ancil_data <- cbind(st_id, cty_id, cty_perc, cty_area)
colnames(ancil_data) <- c("STATEFP", "COUNTYFP", "PCT_CROP", "AREA_SQKM")


#CONSTRUCT FINAL DATASET
df <- cbind(ancil_data, out_data)
#http://data.library.virginia.edu/reshaping-data-from-wide-to-long/
dft <- melt(df, id.vars=c("STATEFP", "COUNTYFP", "PCT_CROP", "AREA_SQKM"),
...
saveRDS(dft, '/nfs/datadrivendroughteffect-data/Data/Final_panels/NDVI_weekly_eco.rds')
cleanup_files(sjob)
