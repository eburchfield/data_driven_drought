...
#create temp folder to store temp data
rasterOptions(tmpdir = "/nfs/datadrivendroughteffect-data/raster_tmp")

# specify directories and variable stuff
...
#specify mask type and time steps per year and years
mask.type <- 'ag'
mask.time <- 'monthly'
year.start <- 1991
year.end <- 2015
...
var.year.avg[,,yy] <- calc(var.year.masked,fun=mean,na.rm=TRUE)[,,1] 
