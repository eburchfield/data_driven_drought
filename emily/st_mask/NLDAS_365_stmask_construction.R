...
# soy_stack <- stack(mget(rep("soy_stack", n)))
# for (i in 1:n) {
...
# 
# writeRaster(soy_stack, filename = "/nfs/datadrivendroughteffect-data/Data/Masks/time_masks/NLDAS_sm_365_soy.nc", format="CDF", overwrite=T)
# writeRaster(wheat_stack, filename = "/nfs/datadrivendroughteffect-data/Data/Masks/time_masks/NLDAS_sm_365_wheat.nc", format="CDF", overwrite=T)
# writeRaster(maize_stack, filename = "/nfs/datadrivendroughteffect-data/Data/Masks/time_masks/NLDAS_sm_365_maize.nc", format="CDF", overwrite=T)
# writeRaster(ag_stack, filename = "/nfs/datadrivendroughteffect-data/Data/Masks/time_masks/NLDAS_sm_agriculture.nc", format="CDF",overwrite=T)
# 
...
soy_stack <- brick("/nfs/datadrivendroughteffect-data/Data/Masks/time_masks/NLDAS_sm_365_soy.nc")
wheat_stack <- brick("/nfs/datadrivendroughteffect-data/Data/Masks/time_masks/NLDAS_sm_365_wheat.nc")
maize_stack <- brick("/nfs/datadrivendroughteffect-data/Data/Masks/time_masks/NLDAS_sm_365_maize.nc")
ag_stack <- brick("/nfs/datadrivendroughteffect-data/Data/Masks/time_masks/NLDAS_sm_365_agriculture.nc")
...
writeRaster(wheat, filename = "/nfs/datadrivendroughteffect-data/Data/Masks/final_masks/NLDAS_365_wheat.nc", format="CDF", overwrite=T)
writeRaster(soy, filename = "/nfs/datadrivendroughteffect-data/Data/Masks/final_masks/NLDAS_365_soy.nc", format="CDF", overwrite=T)
writeRaster(maize, filename = "/nfs/datadrivendroughteffect-data/Data/Masks/final_masks/NLDAS_365_maize.nc", format="CDF", overwrite=T)

#ag/eco flag

test <- raster("/nfs/datadrivendroughteffect-data/Data/Landuse/lulc_bin.tif")
ras_ag <- aggregate(test, fact=30, fun=modal, na.rm=T)
ras_prj <- projectRaster(ras_ag, crs=projection(data_nldas))
ras_crp <- crop(ras_prj, data_nldas)
