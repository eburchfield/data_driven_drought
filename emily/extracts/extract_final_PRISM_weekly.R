...
crop <- "ag" #ag, soy, maize, wheat, eco
...
data_list <- Sys.glob(paste('/nfs/datadrivendroughteffect-data/Data/PRISM/Masked/weekly/ppt/PRISM_weekly_ppt_', crop, '_','*', '.nc',sep=""))
...
fn <- paste('/nfs/datadrivendroughteffect-data/Data/Final_panels/PRISM_weekly_ppt_', crop, '.rds', sep='')
...
#SPECIAL FOR PRISM WEEKLY
week <- rep(seq(1,52,by=1), 35)
week <- sprintf("%02d", week)
year <- as.character(rep(seq(1981,2015, by=1), each=52))
date <- cbind(week, year)
date <- transform(date,wy=paste0(week,year))
cname <- as.character(date$wy)
colnames(out_data)[1:1820] <- cname
...
dft$YEAR <- as.numeric(substr(dft$CNAME, 3, 7))
dft$WEEK <- as.numeric(substr(dft$CNAME, 1, 2))
