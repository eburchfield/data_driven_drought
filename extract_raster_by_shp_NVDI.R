...
extract_data <- function(i) {
  ...
  #extract date info 
  year <- substr(name, 51, 52)
  year <- ifelse(as.numeric(year) > 17, paste("19", year, sep=""), paste("20", year, sep=""))
  doy <- format(strptime(substr(name, 54, 56), format="%j"), format="%m-%d") 
  month <- substr(doy,0,2) #add column
  day <- substr(doy,4,5)
  ...
  for (name in names(ag_data)) {
    ...
    names(year) <- "year"
    names(month) <- "month"
    cty_list <- c(year, month, day, mn, qt)
